    ...
    const poll = async () => {
      try {
        ...
        if (data && data.length > 0) {
          console.log(`📡 Poll: ${data.length} updates found`, data.map((d: any) => d.status));
  ...
  const handleEnterClick = () => {
    ...
    fetch(`${apiUrl}/update`, {
      ...
      .then(data => {
        ...
        // Build updates from API response output as final fallback
        if (data.output && Array.isArray(data.output)) {
          setUpdates((current) => {
            // Only use fallback if polling/realtime didn't deliver WRITING or VERIFIED data
            const hasData = current.some(u => u.status === 'WRITING' || u.status === 'VERIFIED');
            if (hasData) return current;

            console.log('⚠️ No realtime/polling data received, using API fallback');
              ...
              return [
                {
                  ...
                  code: item.old_content || item.new_content || '',
                },
                {
                  id: 1000 + idx * 2 + 1,
                  created_at: new Date().toISOString(),
                  status: 'VERIFIED',
                  message: `✅ Verified ${filename}`,
                  code: item.new_content || '',
                }
              ];
            }).flat();
      ...
      {finished && (
        <div className="max-w-5xl mx-auto">
          <div className="bg-[rgba(30,30,30,0.8)] backdrop-blur-[50px] rounded-[20px] p-16 mb-8 border border-gray-700/50">
            <MultiFileCodeCard
              files={(() => {
                // Helper to extract filename from any status message
                ...
                // Get VERIFIED updates (preferred) or fall back to WRITING updates
                ...
                return finalUpdates.map((update) => {
                  ...
                  // Find the corresponding READING update for this file (original code)
                  ...
                  return {
                    ...
                    new: {
                      name: filename,
                      content: update.code || "",
                      description: verifiedUpdates.length > 0 ? `✅ Verified: ${filename}` : update.message,
                    }
                  };
                });
              })()}
            ...
            <div className="flex items-center justify-center gap-3 mb-8">
              {[
                ...
                const isPast = (() => {
                  ...
                  const currentPhaseIdx = phaseOrder.findIndex(p => 
                    phase.statuses.includes(p)
                  );
                ...
                return (
                  <div key={phase.label} className="flex items-center gap-3">
                    <div className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-500 ${
                      isActive 
                        ? 'bg-white/10 border border-white/30 scale-110' 
                        : isPast 
